...
/**
 * 每个{@link PlanNode} 都持有一个environment<br/>
 * 在此node中循环处理每个tuple时，都清理environment然后重新添加（即一个tuple对应一个干净的environment）。
 * Expression根据environment就可以从tuple取出对应值<br/>
 *
 ...
 *
 * @see Expression#evaluate(Environment)
 *
 * @design (donnie) This class could be applied in several different ways. Any
 *         SELECT clause really could (or should) have its own environment
 *         associated with it, because it will reference tables. In addition, a
 *         derived table (a named subquery in the FROM clause) can also be
 ...
 *         get its parent's environment. Or, we could set all plan-nodes to have
 *         a specific environment, and just manage that assignment process
 *         carefully.
 *         <p>
...
public class Environment {
    ...
    /**
     * Construct a new, empty environment with a (possibly <tt>null</tt>) parent
     * environment.
     */
